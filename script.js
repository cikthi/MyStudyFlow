...
(function() {
	...
	toDoButton.addEventListener('click',function(){
		...
		newList.innerHTML = 
		'<span class="txt"></span> <input type="datetime-local" class="due-input"/><span class="dueTimer"></span><span class="idTask"></span><a class="up" href="#"></a><a class="down" href="#"></a><a class="delete" onclick="delTask(event, this.parentNode)" href="#"></a>';
...
// Update countdown untuk semua task
function updateTimers() {
  var tasks = document.querySelectorAll("li");
  var now = new Date();

  tasks.forEach(function(task) {
    var dueInput = task.querySelector(".due-input");
    var timerEl = task.querySelector(".dueTimer");
    if (!dueInput || !timerEl) return;

    var dueStr = dueInput.value;
    if (!dueStr) {
      timerEl.textContent = "";
      return;
    }
    ...
    var diff = (due - now) / 1000; // dlm saat

    if (diff > 0) {
      var hrs = Math.floor(diff / 3600);
      var mins = Math.floor((diff % 3600) / 60);
      var secs = Math.floor(diff % 60);
      timerEl.textContent = `⏳ ${hrs}j ${mins}m ${secs}s`;

      task.classList.remove("overDue");
      if (diff <= 1800) { // kurang 30 minit
        task.classList.add("dueSoon");
      } else {
        task.classList.remove("dueSoon");
      }
    } else {
...
// Store alert timers so we don’t duplicate them
let overdueTimers = {};

function updateTaskTimes() {
  const now = new Date();

  document.querySelectorAll(".due-input").forEach(input => {
    const li = input.closest("li");
    const taskText = li.querySelector(".txt").innerText;
    const dueDate = input.value ? new Date(input.value) : null;
    ...
    if (dueDate) {
      ...
      if (now > dueDate) {
        ...
        // Start repeating alert if not already running
        if (!overdueTimers[li.id]) {
          overdueTimers[li.id] = setInterval(() => {
            alert("⏰ Task is overdue: " + taskText);
          }, 60000);
          alert("⏰ Task is overdue: " + taskText); // fire once now
        }
      } else {
        li.style.background = "";

        // Stop alert if task is no longer overdue
        if (overdueTimers[li.id]) {
          clearInterval(overdueTimers[li.id]);
          delete overdueTimers[li.id];
        }
      }
    }
  });
...
document.addEventListener("change", function(e) {
  if (e.target.classList.contains("due-input")) {
    ...
    localStorage.setItem("due_" + li.id, dueDate);
  }
});

function restoreDueDates() {
  document.querySelectorAll(".due-input").forEach(input => {
    const li = input.closest("li");
    const savedDate = localStorage.getItem("due_" + li.id);
    if (savedDate) {
...
window.addEventListener("load", () => {
  restoreDueDates(); // restore saved due dates
  setInterval(updateTaskTimes, 1000); // keep updating
  updateTaskTimes(); // run once immediately
});
